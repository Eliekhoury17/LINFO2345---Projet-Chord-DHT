...
-export([launch/1, launch/6, init/6, init/7, loop/6, speak_all/1, reliable_speak_all/1, create_ring/1]).
                ...
                {{_, PredecessorId, _}, {_, _, SuccessorPid}} ->
                    if
                        Key >= PredecessorId, Key < Id ->
                            loop(Index, Id, Successor, Predecessor, maps:put(Key, Value, Map), DirName);
                    ...
                    if
                        Key >= PredecessorId, Key < Id ->
                            case maps:is_key(Key, Map) of
                                true ->
                                    loop(Index, Id, Successor, Predecessor, maps:remove(Key, Map), DirName);
                                false ->
                                    loop(Index, Id, Successor, Predecessor, Map, DirName)
                            end;
                        Id < PredecessorId, Key >= PredecessorId orelse Key < Id ->
